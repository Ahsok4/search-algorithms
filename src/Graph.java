public class Graph {
    ...
    public void removeLink(int node1ID, int node2ID) {
        for (int i = 0; i < nodes.length; i++) {
            if (nodes[i].getId() == node1ID) {
                nodes[i].removeNeighborId(node2ID);
            } else if (nodes[i].getId() == node2ID) {
                nodes[i].removeNeighborId(node1ID);
            }
        }
    }
